...
namespace TestDolbyIO.Comms.Unity
{
    [AddComponentMenu("Dolby.io Comms/Dolby.io Manager", 1)]
    [Inspectable]
    public class TestDolbyIOManager : MonoBehaviour
    {
        ...
        public static TestDolbyIOManager Instance {get; private set; }
        ...
        async void Awake()
        {
            ...
            var token = await configLoader.GetToken();

            _sdk.InitAsync(token, RefreshToken).Wait();
